...
export async function processResponsesStream<TApi extends Api>(
	...
	const createSlot = (outputIndex: number, item: ResponseOutputItem): ResponsesOutputSlot | undefined => {
		...
		if (item.type === "function_call") {
			const fallbackSeed = `${item.name}|${item.arguments}|${output.content.length}`;
			const rawItemId =
				typeof item.id === "string" && item.id.trim() ? item.id : fallbackResponsesItemId(fallbackSeed);
			let itemId = normalizeResponsesIdPart(rawItemId, fallbackResponsesItemId(fallbackSeed));
			if (!itemId.startsWith("fc_")) {
			...
			const callId = normalizeResponsesIdPart(
				item.call_id,
				fallbackResponsesCallId(`${itemId}|${item.name}|${item.arguments}|${output.content.length}`),
			);
